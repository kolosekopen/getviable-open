...
$.jcoverflip = {
	...
	getItemFromElement: function( element ){
		...
		if( item.size( ) == 0 ){
			...
			/* NEBOJSA: MOVING ELEMENT TO THE CENTER! */
			/*{ element: item, index: item.data( 'jcoverflip__index' ) } */ 
			return null;
		}
	}
};
...
$.widget( 'ui.jcoverflip', {
	...
	next: function( by, wrapAround, callback, originalEvent ){
		var nextAux = function(obj, itemsRight) {
			if (obj.items().length == (obj.current() + itemsRight)) {
				return null;
			} else {
			  return obj._nextAux( params.by, params.wrapAround, params.callback, params.originalEvent, 'next' );
			}
		}
		...
		// Nebojsa: Hardcoded that 3rd element is in the center!!!
		// Ninoslav: Hardcoded that n-th element is in the center, based on the viewport width!
		// Note: Viewport width is changed by twitter bootstrap scripts at some point during shrinking,
		// so it is not 100% correct, but it is close...
		var flip_width = $('#flip').width();

		if (flip_width > 514) {
			nextAux(this, 3);
		} else if (flip_width <= 514 && flip_width > 259) {
			nextAux(this, 2);
		} else if (flip_width <= 259) {
			nextAux(this, 1);
		}
	},
...
$.ui.jcoverflip.defaults = {
	...
	beforeCss: function( el, container, offset ){
		return [
			$.jcoverflip.animationElement( el, { left: ( container.width( )/2 - 250 - 170*offset )+'px', bottom: '10px' }, { } ),
			$.jcoverflip.animationElement( el.find( 'img' ), { opacity: 1,  width: '160px', height: '94px' }, {} )
		];
	},
	afterCss: function( el, container, offset ){
		return [
			$.jcoverflip.animationElement( el, { left: ( container.width( )/2 + 100 + 170*offset )+'px', bottom: '10px' }, { } ),
			$.jcoverflip.animationElement( el.find( 'img' ), { opacity: 1, width: '160px', height: '94px'  }, {} )
		];
	},
	currentCss: function( el, container ){
		return [
			$.jcoverflip.animationElement( el, { left:( container.width( )/2 - 75 )+'px', bottom: '10px' }, { } ),
			$.jcoverflip.animationElement( el.find( 'img' ), { opacity: 1,  width: '160px', height: '94px' }, { } )
		];
	},
