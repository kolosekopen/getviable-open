...
module Surveyor
  module Helpers
    module SurveyorHelperCustomMethods
      ...
      def previous_section
        ...
        submit_tag(t('surveyor.previous_section').html_safe, :name => "section[#{@sections[@sections.index(@section)-1].id}]", :class => "btn btn-warning btn-save") unless @sections.first == @section
      end
      def next_section
        # use copy in memory instead of making extra db calls
        @sections.last == @section ? "" : submit_tag(t('surveyor.next_section').html_safe, :name => "section[#{@sections[@sections.index(@section)+1].id}]", :class => "btn btn-warning btn-save")
      end

      def save_section
        # use copy in memory instead of making extra db calls
        submit_tag(t('surveyor.save_section').html_safe, :name => "section[#{@sections[@sections.index(@section)].id}]", :class => "btn btn-success btn-save")
      end

      # Questions
      def q_text(obj, context=nil)
        if obj.display_type == "label" && obj.try(:question_reference_id)
          reference_answer = Response.where(:response_set_id => @response_set.id, :question_id => obj.question_reference_id).first
          return "#{obj.render_question_text(context)}: <span class=\"bigideaname\">#{reference_answer.try(:string_value)}</span>"
        end
        ...
        #TODO: Refactor this part. Question reference id doesn't have in question_groups table
        if (!obj.class.name.include? "QuestionGroup") && !obj.try(:question_reference_id).nil?  
          reference_answers = Response.where(:response_set_id => @response_set.id, :question_id => obj.question_reference_id)
          answers = []
          reference_answers.each do |reference_answer|
            #If answer is a string from input field || If answer is from checkboxes, radio buttons etc.
            answers << (reference_answer.try(:string_value) || reference_answer.answer.text)
          end
          "#{obj.render_question_text(context)}: <span class=\"bigideaname\">#{answers.each{|a| a}}</span>"       
        else
           ##{next_question_number(obj)}  < Next question number is not displaying!
...
module SurveyorHelper
  ...
  include Surveyor::Helpers::SurveyorHelperCustomMethods
